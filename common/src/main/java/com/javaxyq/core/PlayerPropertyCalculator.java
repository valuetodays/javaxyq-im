...
public class PlayerPropertyCalculator {
    ...
    private static int formula_harm(PlayerVO attrs, double hurt, int index) {
        for(int level = 0; level < ((Integer)attrs.attrsLevel.get("伤害")).intValue(); ++level) {
            if(level == 0) {
                hurt += 加成系数1[index];
            }

            hurt += 加成系数2[index] + 加成系数3[index] * (double)((Integer)attrs.attrsLevel.get("伤害")).intValue();
        }
    ...
    private static int formula_defense(PlayerVO attrs, double defense) {
        for(int level = 0; level < ((Integer)attrs.attrsLevel.get("防御")).intValue(); ++level) {
            defense += 1.01D + 0.014D * (double)((Integer)attrs.attrsLevel.get("防御")).intValue();
        }
    ...
    private static int formula_wakan(PlayerVO attrs, double wakan) {
        for(int level = 0; level < ((Integer)attrs.attrsLevel.get("灵力")).intValue(); ++level) {
            wakan += 0.5D + 0.009D * (double)((Integer)attrs.attrsLevel.get("灵力")).intValue();
        }
    ...
    private static int formula_dodge(PlayerVO attrs, double dodge) {
        for(int level = 0; level < ((Integer)attrs.attrsLevel.get("躲避")).intValue(); ++level) {
            dodge += 2.02D + 0.02D * (double)((Integer)attrs.attrsLevel.get("躲避")).intValue();
        }
    ...
    public static int calc_speed(PlayerVO attrs) {
        ...
        if(StringUtils.equals("女儿村", attrs.school)) {
            for(int level = 0; level < ((Integer)attrs.attrsLevel.get("速度")).intValue(); ++level) {
                speed += 0.68D + 0.005D * (double)((Integer)attrs.attrsLevel.get("速度")).intValue();
            }
        }
    ...
    public static int calc_hitrate(PlayerVO attrs) {
        ...
        if(inArray(人族, attrs.character)) {
            ...
            if(StringUtils.equals("大唐官府", attrs.school)) {
                for(level = 0; level < ((Integer)attrs.attrsLevel.get("命中")).intValue(); ++level) {
                    hitrate += 1.01D + 0.02D * (double)((Integer)attrs.attrsLevel.get("命中")).intValue();
                }
            }
    ...
    public static int calc_气血(PlayerVO attrs) {
        if(inArray(人族, attrs.character)) {
            ...
            if(StringUtils.equals("天宫", attrs.school)) {
                for(int level = 0; level < ((Integer)attrs.attrsLevel.get("HP")).intValue(); ++level) {
                    hp += hp * 0.003D * (double)((Integer)attrs.attrsLevel.get("HP")).intValue();
                }
            }
    ...
    public static int calc_魔法(PlayerVO attrs) {
        if(inArray(人族, attrs.character)) {
            ...
            if(StringUtils.equals("五庄观", attrs.school)) {
                for(int level = 0; level < ((Integer)attrs.attrsLevel.get("MP")).intValue(); ++level) {
                    System.out.println("mp is:" + mp);
                    System.out.println("attrsLevel is:" + attrs.attrsLevel.get("MP"));
                    mp += mp * 0.005D * (double)((Integer)attrs.attrsLevel.get("MP")).intValue();
                }
            }
    ...
    public static Object invokeMethod(String mName, Object arg) {
        try {
            Method e = PlayerPropertyCalculator.class.getMethod(mName, new Class[]{arg.getClass()});
            return e.invoke(PlayerPropertyCalculator.class, new Object[]{arg});
